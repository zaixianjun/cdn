...
'cms':{//系统
	'nav': function($id){//主导航高亮
		$id = $('.navbar[data-dir]').attr('data-dir');
		$($id).addClass("active");
	},
	'pre':function(){//代码高亮
		...
		if($("pre code").length){
			...
			$("<link>").attr({ rel: "stylesheet",type: "text/css",href: "/Public/cdn/highlight/highlight.min.css"}).appendTo("head");
			$.getScript("/Public/cdn/highlight/highlight.min.js", function(){
	...
	'flickity':function(){//手机滑动
		if($(".ff-gallery").length){
			...
			$("<link>").attr({
				rel: "stylesheet",
				type: "text/css",
				href: "/Public/cdn/flickity/flickity.min.css"
			}).appendTo("head");
			$.getScript("/Public/cdn/flickity/flickity.pkgd.min.js", function(){
...
'scroll':{//滚动条
	...
	'totop':function($id, $top){ //返回顶部
		// $id:dc-totop $top:偏移值
		$('body').append('<a href="#top" class="'+$id+'" id="'+$id+'"><i class="iconfont iconfont-top">&#xe67e;</i></a>');
		$(window).bind('scroll', function(){
...
'language':{//简繁转换
	's2t':function(){
		if(feifei.browser.language=='zh-hk' || feifei.browser.language=='zh-tw'){
			$.getScript("/Public/cdn/s2t/s2t.min.js", function(data, status, jqxhr) {
...
'image': {//图片
	'lazyload': function(){//延迟加载
		...
		$.getScript("/Public/cdn/lazyload/jquery.lazyload.min.js", function(response, status) {
			$("img.ff-img").lazyload({
	...
	'qrcode': function(){//生成二维码
		...
		$(".glyphicon-phone").on('show.bs.popover', function () {
			$(".glyphicon-phone").attr('data-content','<img class="ff-qrcode" src="/Public/images/qrcode/1.0/?w=150&h=150&url='+encodeURIComponent(feifei.browser.url)+'"/>');
		})
	},
	'vcode':function(){//安全码
...
'search': {//搜索
	...
	'autocomplete': function(){//3.5增加搜索结果控制
		...
		if( $limit > 0){
			...
			$.getScript("/Public/cdn/autocomplete/jquery.autocomplete.min.js", function(response, status) {
	...
	'get':function(){
		...
		if($(".history-record-get").length){
			/*$.get(cms.root+'index.php?g=home&m=record&a=vod&sid=1', function(data){
				if(data == ''){
					data = '<p class="text-center text-light">暂无观看记录</p>';
				}else{
			...
			var data="";
			var oldlog=feifei.record.getCookie("logdata");
			oldlog=oldlog==null?"":oldlog;
			if(oldlog!=""){
				data='<ul class="history-list">';
				var arr=oldlog.split('$');
				for(var i=0;i<arr.length;i++){
					if(arr[i]!=''){
					var arr1=arr[i].split('|');
					var url0=arr1[5];
					var url1="/v/"+arr1[2]+"/"+arr1[0]+"-"+arr1[1]+".html";
					data+='<li><a class="text-blue" href="'+url0+'">'+arr1[3]+'</a><span class="sep">-</span><a href="'+url1+'">'+arr1[4]+'</a></li>';
					}
				}
				data+="</ul>";
			}else{
...
　　　　document.cookie=c_name+ "=" + escape(value) + ((expiredays==null) ? "" : ";expires="+exdate.toGMTString()+";domain=.shoupa.com;path=/");
 　　},
	 'getCookie':function (name){
			...
			var cval=feifei.record.getCookie(name);
			if(cval!=null){
	...
	'set':function(){//用户互动记录
		...
		if($(".ff-record-set[data-type=1]").eq(0).attr('data-sid')){
			...
			var logdata=$this.attr("data-id-sid")+"|"+$this.attr("data-id-pid")+"|"+$this.attr("data-id")+"|"+$this.attr("vod_name")+"|"+$this.attr("play_title")+"|"+$this.attr("url");
			var oldlog=feifei.record.getCookie("logdata");
			//console.info(document.cookie);
			//console.info(oldlog);
			oldlog=oldlog==null?"":oldlog;
			logdata=logdata+"$"+oldlog;
			var arr=feifei.record.diffarr(logdata);
			if(arr.length>0){
				feifei.record.setCookie("logdata",arr.join("$"),30);
			}
...
'score': {//评分
	'raty': function(){
		if( $('.ff-score').length ){
			...
			//
			$.getScript("/Public/cdn/raty/jquery.raty.min.js", function(response, status) {
...
//				$id = $(".ff-share");
//			}
//			$($id).html('<div class="bdsharebuttonbox"><a href="#" class="bds_qzone" data-cmd="qzone" title="分享到QQ空间"></a><a href="#" class="bds_weixin" data-cmd="weixin" title="分享到微信"></a><a href="#" class="bds_sqq" data-cmd="sqq" title="分享到QQ好友"></a><a href="#" class="bds_tsina" data-cmd="tsina" title="分享到新浪微博"></a><a href="#" class="bds_copy" data-cmd="copy" title="分享到复制网址"></a><a href="#" class="bds_more" data-cmd="more"></a></div>');
...
'scenario': {//分集剧情
	'load':function(){
		...
		$('.detail .detail-tab li:first > a').tab('show');
	}
},
'playurl': {//播放地址
	'download': function(){
		if($(".ff-playurl-down").length){
			$.getScript("/Public/cdn/download/xunlei.js",function(){
				//3.4增加鼠标点击去掉文件名标题
...
$(document).ready(function(){
	...
	feifei.forum.load();
	feifei.vcode.load();
	feifei.scroll.totop('backtop',10);
	feifei.record.load();//
	feifei.hits.load();
	//feifei.share.baidu();
});