var $MH={
	...
	fontlist: function (){
		...
		for(var i=0;i<a.video.length;i++){
			if(c>$MH.limit){break;}
			if(a.video[i].link && a.video[i].pic && a.video[i].name){
			img_li += "<li><a style='font-size: 14px' class=\"text-blue\" href=\"" + a.video[i].link + "\" target=\"_blank\">" + a.video[i].name + "</a><span class=\"sep\">-</span><a href=\"" + a.video[i].plink + "\">"+a.video[i].part+"</a></li>"
				c++;
			}
		}
		img_li = img_li != "" ? img_li : '<li style="width:100%;text-align:center;line-height:'+($MH.height-25)+'px;color:red;list-style:none">\u6CA1\u6709\u8BB0\u5F55</li>';
		return "<div id=\"mh-ul\"><ul class=\"history-list\">" + img_li + "</ul><div style=\"clear:both\"></div></div></div>";
	...
	recordHistory: function(video){
		if(video.link.indexOf('http://')==-1 || window.max_Player_File || video.part.indexOf('{') != -1) return;
		var a = $MH.getCookie('HISTORY'), b = new Array(), c = 1;
		if(a !='' && a != null && a != 'null'){
			a = $MH.tryjosn(a);
			for(var i=0;i<a.video.length;i++){
				if(c>$MH.limit){break;}
				if(video.link != a.video[i].link && a.video[i].pic){b.push('{"name":"'+ $MH.u8(a.video[i].name) +'","link":"'+ $MH.u8(a.video[i].link) +'","pic":"'+ $MH.u8(a.video[i].pic) +'","part":"'+ $MH.u8(a.video[i].part) +'","plink":"'+ $MH.u8(a.video[i].plink) +'"}');c++;}
			}
		}
		b.unshift('{"name":"'+ $MH.u8(video.name) +'","link":"'+ $MH.u8(video.link) +'","pic":"'+ $MH.u8(video.pic) +'","part":"'+ $MH.u8(video.part) +'","plink":"'+ $MH.u8(video.plink) +'"}');
